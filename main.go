...
func (t Training) meanSpeed() float64 {
	// вставьте ваш код ниже
	hours := t.Duration.Minutes() / MinInHours
	if hours == 0 { 
		return 0
	}
...
func (r Running) Calories() float64 {
	speed := r.meanSpeed() // средняя скорость в км/ч
	hours := r.Duration.Minutes() / MinInHours
...
// Calories возвращает количество калорий при ходьбе.
func (w Walking) Calories() float64 {
	speedMsec := w.meanSpeed() * KmHInMsec
	heightMeters := w.Height / CmInM
	hours := w.Duration.Minutes() / MinInHours
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speedMsec, 2)/heightMeters)*CaloriesSpeedHeightMultiplier*w.Weight) * hours * MinInHours)
}

// TrainingInfo возвращает структуру InfoMessage для ходьбы.
...
func (s Swimming) meanSpeed() float64 {
	hours := s.Duration.Minutes() / MinInHours
	return float64(s.LengthPool*s.CountPool) / MInKm / hours
}

// Calories возвращает количество калорий при плавании.
...
// TrainingInfo возвращает структуру InfoMessage для плавания.
